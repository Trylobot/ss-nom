...
public class CampaignArmadaResourceSharingController implements EveryFrameScript
{
    ...
    CampaignFleetAPI leader_fleet = armada.getLeaderFleet();
		...
		{
			TrylobotUtils.debug("redistributing "+available_crew+" crew from "+generous_fleets.size()+" fleets to "+jeopardized_fleets.size()+" fleets.");
			redistribute_crew( available_crew, generous_fleets, needed_crew, jeopardized_fleets );
		}
		if( available_supplies > 0 && needed_supplies > 0 )
		{
			TrylobotUtils.debug("redistributing "+available_supplies+" supplies from "+generous_fleets.size()+" fleets to "+jeopardized_fleets.size()+" fleets.");
		...
		{
			TrylobotUtils.debug("redistributing "+available_fuel+" fuel from "+generous_fleets.size()+" fleets to "+jeopardized_fleets.size()+" fleets.");
	...
	private void redistribute_crew( float available_crew, List donor_fleets, float needed_crew, List benefactor_fleets )
	{
		...
		for( int i = 0; i < donor_fleets.size(); ++i )
		{
			...
			available_GREEN = cargo.getCrew();
			if( Math.min( take_amount, available_GREEN ) > 0 )
			{
				take_actual = Math.min( take_amount, available_GREEN );
				cargo.removeCrew( take_actual );
				take_amount -= take_actual;
			}
		}
		...
		for( int i = 0; i < benefactor_fleets.size(); ++i )
		{
			...
			//////////////////
			// give crew to donor
			give_actual = (int)Math.ceil( give_ratio * take_actual );
			cargo.addCrew( give_actual );
    }
	}
    ...
    CampaignFleetAPI[] escort_fleets = armada.getEscortFleets();
		...
		for( int i = 0; i < escort_fleets.length; ++i )
		{
			...
			if( needed_crew > 0 )
			{
				fleet.getCargo().addCrew( (int)Math.ceil( needed_crew ));
		...
		if( VERBOSE_LOGGING && cheated_fleets > 0 )
			TrylobotUtils.debug("CONJURED resources for "+cheated_fleets+" fleets; "+cheated_crew+" crew, "+cheated_supplies+" supplies, and "+cheated_fuel+" fuel");
	}
	...
	private void log_stats_verbose()
	{
		...
		TrylobotUtils.debug(sb.toString());
	}
