package data.campaign.rulecmd.salvage;
 ...
 * 
 */
public class NomadsSurveyData extends BaseCommandPlugin
{
  protected String commodity = Commodities.SURVEY_DATA;
	...
	protected boolean buysCommodity;
  ...
  @Override
	public boolean execute(String ruleId, InteractionDialogAPI dialog, List<Token> params, Map<String, MemoryAPI> memoryMap) {
      ...
      return false;
		...
		buysCommodity = faction.getCustomBoolean("buysSurveyData");
		valueMult = faction.getCustomFloat("SurveyDataValueMult");
		repMult = faction.getCustomFloat("SurveyDataRepMult");
		
    switch (command) {
      case "selectSurveyData":
        selectCommodityToGive();
        break;
      case "playerHasSurveyData":
        return playerHas(commodity);
      case "personCanAcceptSurveyData":
        return personCanAcceptCommodity();
    }
		
		return true;
	}

	protected boolean personCanAcceptCommodity() {
    // Must be a person of a nomad fleet containing an Oasis
    ...
    boolean hasOasis = false;
    for (FleetMemberAPI ship : interactionFleet.getMembersWithFightersCopy()) {
	...
	protected void selectCommodityToGive() {
		...
		dialog.showCargoPickerDialog("Select Survey Data to transfer", "Confirm", "Cancel", true, width, copy, 
      ...
      new CargoPickerListener()
      {
        public void pickedCargo(CargoAPI cargo)
        {
          ...
          float bounty = computeCreditValue(cargo, commodity);
          float repChange = computeReputationValue(cargo, commodity);
          ...
          FireBest.fire(null, dialog, memoryMap, "NomadsSurveyDataTurnedIn");
        }
        public void cancelledCargoSelection() {
        }
        public void recreateTextPanel(TooltipMakerAPI panel, CargoAPI cargo, CargoStackAPI pickedUp, boolean pickedUpFromSource, CargoAPI combined)
        {
          float bounty = computeCreditValue(combined, commodity);
          float repChange = computeReputationValue(combined, commodity);
          ...
          panel.addPara("Compared to dealing with other factions, turning Survey Data in to " + 
          ...
          panel.addPara("If you turn in this Survey Data, you will receive a %s bounty " +
	...
	protected float computeCreditValue(CargoAPI cargo, String commodity_demandClass) {
		float bounty = 0;
		for (CargoStackAPI stack : cargo.getStacksCopy()) {
			CommoditySpecAPI spec = stack.getResourceIfResource();
			if (spec != null && spec.getDemandClass().equals(commodity_demandClass)) {
	...
	protected float computeReputationValue(CargoAPI cargo, String commodity_demandClass) {
		float rep = 0;
		for (CargoStackAPI stack : cargo.getStacksCopy()) {
			CommoditySpecAPI spec = stack.getResourceIfResource();
			if (spec != null && spec.getDemandClass().equals(commodity_demandClass)) {
	...
	public static float getBaseRepValue(String commodity_type) {
    switch (commodity_type) {
      case Commodities.SURVEY_DATA_5: return 5f;
      case Commodities.SURVEY_DATA_4: return 2.5f;
      case Commodities.SURVEY_DATA_3: return 1.25f;
      case Commodities.SURVEY_DATA_2: return 1.125f;
      case Commodities.SURVEY_DATA_1: return 1f;
      default:                        return 1f;
    }
	}
	
	protected boolean playerHas(String commodity_demandClass) {
		for (CargoStackAPI stack : playerCargo.getStacksCopy()) {
			CommoditySpecAPI spec = stack.getResourceIfResource();
			if (spec != null && spec.getDemandClass().equals(commodity_demandClass)) {
				return true;
